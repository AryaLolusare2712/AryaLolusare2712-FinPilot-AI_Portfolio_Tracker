...
with tab_ai:
    if not llm.available:
        st.warning("⚠️ Gemini AI is offline. Add `GEMINI_API_KEY` to your `.env` file. "
...
with tab_chat:
    if not llm.available:
        st.warning("⚠️ Gemini AI offline. Add `GEMINI_API_KEY` to `.env` to use chat.")
...
with tab_news:
    ...
    if not articles:
        st.info("No news available. Add `NEWS_API_KEY` to `.env` for live market news.")
    else:
